...
def generate_prediction(sign: str) -> str:
    prompt = f"""
Составь очень короткий гороскоп-предсказание для знака {sign}.
Формат: одно предложение, максимум 10–12 слов.
Стиль: ироничный, остроумный, как короткая шутка или мем.
Темы: настроение, любовь, деньги, лень, бытовые мелочи, случайные события.
Не используй сюжеты про офис, работу, начальство или коллег.
Не начинай текст со слова "Сегодня".
Пиши только на русском языке, полностью исключи английские слова.
Не используй слова «планеты», «астрология».
Примеры:
- Вы милашки, можете уже даже не пытаться доказывать обратное.
- Опять у вас с карты исчезают деньги и непонятно, кто их тратит.
- Сложно быть совершенством. Но вы стараетесь.
"""
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": prompt}],
        temperature=0.9,
        max_tokens=40  # ограничение на длину
    )
