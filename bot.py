...
def generate_prediction(sign: str) -> str:
    prompt = f"""
Составь короткий гороскоп-предсказание для знака {sign}.
Формат: одно предложение.
Стиль: ироничный, остроумный, дружеский, как шутка или мем.
Темы: настроение, любовь, деньги, лень, удача, бытовые мелочи, случайные события.
...
Не используй слова «звёзды», «планеты», «астрология».
Примеры:
- Пора попробовать в своей жизни что-то новое, например, поставить людей на место.
- Вы милашки, можете уже даже не пытаться доказывать обратное.
- Опять у вас с карты исчезают деньги и непонятно, кто вообще их за вас тратит.
"""
